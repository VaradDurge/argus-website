...
export function Header() {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 mx-auto w-full max-w-5xl border border-transparent md:rounded-xl md:transition-all md:duration-500 md:ease-out",
        {
          "md:top-4 md:max-w-4xl": scrolled && !open,
          "bg-background/60": open,
        }
      )}
    >
      {scrolled && !open && (
        <div
          className="absolute inset-0 rounded-xl hidden md:block"
          style={{
            background:
              "linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.01) 50%, rgba(82,39,255,0.03) 100%)",
            backdropFilter: "blur(20px) saturate(1.4)",
            WebkitBackdropFilter: "blur(20px) saturate(1.4)",
            border: "1px solid rgba(255,255,255,0.06)",
            boxShadow:
              "0 8px 32px rgba(0,0,0,0.4), 0 0 0 0.5px rgba(255,255,255,0.04) inset, 0 1px 0 rgba(255,255,255,0.05) inset",
          }}
        />
      )}
      ...
      {scrolled && !open && (
        <div
          className="absolute bottom-0 left-[10%] right-[10%] h-px hidden md:block"
          style={{
            background:
              "linear-gradient(90deg, transparent, rgba(82,39,255,0.3), rgba(227,141,224,0.2), transparent)",
          }}
        />
      )}
      ...
      <nav
        className={cn(
          "relative z-10 flex h-14 w-full items-center justify-between px-5 md:h-12 md:transition-all md:duration-500 md:ease-out",
          { "md:px-4": scrolled }
        )}
      >
        <Link
          ...
          className="group relative min-w-[132px]"
        >
          <span
            className="text-sm font-semibold tracking-[0.08em] uppercase transition-all duration-300"
            style={{
              background:
          ...
          <span
            ...
            style={{
              background:
                "linear-gradient(90deg, rgba(82,39,255,0.6), rgba(227,141,224,0.4), transparent)",
            }}
          />
        </Link>
          ...
          {links.map((link) => (
            <a
              ...
              className={cn(
                ...
                "relative text-[13px] text-white/50 transition-all duration-300 hover:text-white/80 hover:bg-white/[0.04]"
              )}
              href={link.href}
      ...
      <div
        className={cn(
          "bg-background/90 fixed top-14 right-0 bottom-0 left-0 z-50 overflow-hidden border-y md:hidden",
